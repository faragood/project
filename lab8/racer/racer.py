...
def game(xx, yy, font, roads):
    ...
    coin = pygame.image.load("images/coin.png") #load coin 
    coin = pygame.transform.scale(coin, (coin.get_size()[0]//10, coin.get_size()[1]//10)) #set size for coin
    ...
    def blit_coins(coins):
        ...
        for i in coins:
            coin_rect = coin.get_rect(topleft = (xx//(roads)*i[0] - coin.get_size()[0] - 5, i[1])) #set new position for coin
            screen.blit(coin, coin_rect)#blit coin
    ...
    coins = [] #list of coins [[road, (y coordinate)], ...]
    ...
    time_until_max_dif = 600 #time until game will reach maximum difficulty (600sec)
    speed_coef = (max_speed - speed)/(time_until_max_dif*100) #variable for adding to speed based on time_until_max_dif
    appear_rand_time_range_coef = (appear_rand_time_range - min_appear_rand_time_range)/(time_until_max_dif*100) #variable for taking away from appear_rand_time_range based on time_until_max_dif
    ...
    while gameOn:
        ...
        for i in range(len(coins)-1):
            if (((yy/1.1 - player_car0.get_size()[1]) - coins[i][1])**2 + ((xx//(roads)*coins[i][0] - coin.get_size()[0] - 5) - (x))**2)*(1/2) <= 50**2:
                del coins[i]
                coin_score+=1 #add 1 to coin score
        ...
        if appear_rand_time_range >= min_appear_rand_time_range:
